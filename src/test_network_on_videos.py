...
'''
{
    first version of two-stream network, this is specific justified to load videos 
    from NTU RGB+D Dataset and test the network performance.
    Inout Path is VIDEO_PATH. Once defined, the system will read videos in this folder and make prdiction. 
    The outputs are images with skeletons information, skeletons as txt file and predicted scores. change it in config/config.json
    Input:
        MODEL_PATH
        VIDEO_PATH
...
with open(ROOT + 'config/config.json') as json_config_file:
    ...
    # VIDEO_PATH = 'C:/Users/Kun/tf_test/Human_Action_Recognition/NTU/S010/Waving_023/'
...
def main_function(VIDEO_PATH):
    ...
    sFile_Names = uti_commons.read_all_file_names(VIDEO_PATH, bSort_Lists=True)
    ...
    for i in range(iNumber_of_Files):
        SRC_PATH = sFile_Names[i]


        # select the data source
        images_loader = uti_images_io.Read_Images_From_Video(SRC_PATH)
        ...
        STAMPLE = SRC_PATH[-28 : -8]

        TEST_RESULTS_FOLDER = TEST_OUTPUTS + 'TEST_'  + STAMPLE + '/scores/'
        TEST_SKELETONS_FOLDER = TEST_OUTPUTS + 'TEST_'  + STAMPLE + '/skeletons/'
        TEST_IMAGES_FOLDER = TEST_OUTPUTS + 'TEST_'  + STAMPLE + '/images/'
